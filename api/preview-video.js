...
// Isolated test endpoint for a single guildVideo document by slug — exact
// mirror of api/preview-story.js, deliberately kept separate from
// api/preview-videos.js (the three-section listing page's endpoint). Not
// wired into the-work/videos/index.html or renderVideoCard in any way.
...
export default async function handler(req, res) {
  ...
  try {
    const result = await client.fetch(
      `*[_type == "guildVideo" && slug.current == $slug][0]`,
      { slug }
    );
